...
/**
 * A class for extending the capabilities provided by the DescriptiveStatistics class of Apache Commons Math library
 */
public class CustomDescriptiveStatistics extends DescriptiveStatistics {
    ...
    public void updateStandardDeviation() {
        if (previousStd == null) {
            ...
            double temp =
                    (windowSize - 2) * Math.pow(previousStd, 2) + (windowSize - 1) * Math.pow((previousMean - mean), 2)
                            + Math.pow((previousMean - mean), 2);
